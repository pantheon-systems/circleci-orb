...
# Manually releasing a prod (semantic) version of this Orb

- Make sure you have the circleci cli installed (`brew install --ignore-dependencies circleci`) (`--ignore-dependencies` is used if you have Docker for Mac installed)
- Make sure the cli is correctly setup: `circleci setup`
- Checkout the tag to deploy (e.g. `git checkout 0.7.0`)
- Run the orb publish command like this (adjust to the right version): `circleci orb publish src/@orb.yml pantheon-systems/pantheon@0.7.0`
